...
**如果无法从缓存中找到，则可以通过请求域名服务器的方式去解析域名。**
...
java.lang.UnsupportedOperationException
	...
	at com.two.TrialError.arraysAsList(TrialError.java:19)
    // ....
```



**Arrays.asList()方法的内部实现：**
...
**该内部类并为对add()方法做出具体的实现：**

<img src="assets/image-20210408103418304.png" alt="image-20210408103418304" style="zoom:90%;" />

**AbstractList抽象类对add()方法的实现：**
...
**对线程的异常处理方式有两种，`Try/catch`、通过`UncaughtExceptionHandler`接口实现。**

**1. try/cath**

```java
@Test
public void uncaughtExceptionHanlderTest() {
    Thread t = new Thread() {
        @Override
        public void run() {
            try {
                ...
                System.out.println(this.getName() + " : (catch) " + e.getMessage());
            }
        }
    };
...
**3. 设置Thread全局默认UnCaughtExceptionHandler**
...
> 使用Executors创建线程时，还可以在ThreadFactory（`new ThreadFactory() {...}`）中设置。
>
> 线程工厂的作用：涉及到用户如何自定义创建线程；用户也能够在线程工厂中定义为捕获异常的处理器。
...
疫情期间需要开发一系列疫情打卡的相关功能，收集学生每日的健康状态与位置信息。出现问题的原因来源于项目内部的一个特殊需求：通过Excel表导出至少一周内的全校学生打卡情况。

**出现在堆区的OOM异常**

在有一天导出学生数据的接口突然失效了。

通过登录服务器查看日志，发现了一处OOM异常：从数据库查询出一段时间的数据之后，会将数据存入一个ArrayList当中，在将ArrayList返回到Servlet层通过IO流发送出去。OOM异常就发生在获取结果集的过程中。

**分析问题**

通过JVisualVM监控到当读取数据的时候，Java堆内存占用量暴增，且频繁发生GC。

通过计算发现从数据库读出的数据量有10万多行的，占用数据量已经超过了Java堆的大小。

**解决方案**

采用JDBC的stream流的方式，当读取完一部分之后再读取另一部分，并重用一个List。当一个List存满制定数量之后直接通过IO流发送出去，再去数据源中获取数据。将10万行数据分开获取、下载。
...
以班级为单位做缓存、对于班级中个别课表不同的做单独的缓存。

标准课表：统计一个班级中重复数最多的课表将其缓存起来，并在每天凌晨刷新一次缓存。

分页返回。不分页返回太多会影响带宽，返回太少还造成同样的运算没能尽到最大的效果。
...
在之前规范未统一起来之前，写了很多连接数据库的配置类。通过静态代理的方式，让所有的配置类都引用同一个连接池配置类，并加上废弃注解。
